...
アプリ内の**AIチャット**は右下の AI ボタンからいつでも呼び出せる。
...
- **チャット**: 右下 AI ボタンまたは AI タブから `POST /api/ask`。回答表示と、依頼の実行（予定追加など）
- **AI タブ**: チャットが主役。上部の「今日の要約」「予定・課題」は折りたたみカードで、
  展開時は最大320dpに収めて中身をスクロール（長文でもチャット欄を潰さない）
...
| 画面（AIチャット含む） | `app/.../MainActivity.kt`, `app/.../ui/MainViewModel.kt` |
...
- **アカウント**: `server/accounts.example.json` を `server/accounts.json` にコピーし、`email` / `token` /（任意）`lineUserId` を登録
- **Gemini**: 各ユーザーが自分の API キーをダッシュボードの「アカウント」タブで登録すると抽出・要約・AIチャットが有効化（サーバー共通の `GEMINI_API_KEY` は廃止）
