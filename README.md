...
アプリ内のチャットは右下の AI ボタンからいつでも呼び出せる。
...
- **チャット**: 右下 AI ボタンまたはチャットタブから `POST /api/ask`。回答表示と、依頼の実行（予定追加など）
...
| 画面（チャット含む） | `app/.../MainActivity.kt`, `app/.../ui/MainViewModel.kt` |
...
- **Gemini**: `GEMINI_API_KEY` を設定すると抽出・要約・チャットが有効化
